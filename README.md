# Student database using C

A simple terminal-based student database system for **Govt.Polytechnic College, Nizamabad**, written in C.
The program supports student record entry, viewing, and updates for multiple branches

## Features
- Add student records branch-wise
- View student records stored in files
- Append new records to existing files
- Uses file handling in C('.txt' files)
- Works with multiple departments (CS, EC, EE, ME, ES, CE)

## Supported Branches
- CS (Computer Science)
- EC (Electronnics and communication)
- EE (Electrical)
- ME (Mechanical)
- ES (Embeded systems)
- CE (Civil)

## How to run
#### Open Terinal and run these commands
- git clone https://github.com/Abhiram-Dashewar/student-database-C
- cd student-database-C
- gcc student_database.c
- .\a.exe

## Requriments
- GCC Compalier (Linux/Mac) or Windows(VS code)
- Git installed on your system

---
